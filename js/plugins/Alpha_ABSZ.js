/*:
 * @plugindesc (v.0.8.5)[BASIC] Active Battle System
 ...
 */
/*:ru
 * @plugindesc (v.0.8.5)[BASIC] Активная боевая система
 * @author Pheonix KageDesu
 * @target MZ MV
 * @url https://kdworkshop.net/plugins/alpha-abs-z/
 *
...
//Plugin KDCore builded by PKD PluginBuilder 2.1 - 07.10.2022
...
AASkill2 = class AASkill2 {
  ...
  isRepeatableOnUse() {
    return this.repeatOnUse > 1;
  }

  isRequireStateOnUser() {
    return this.usableIfState > 0;
  }

  isRequireStateOnTarget() {
    return this.usableIfTargetState > 0;
  }
  ...
  getImpulsePower() {
    return this._getSafeEValue(this.impulse);
  }

  //TODO: Перевести все EVal на этот метод
  _getSafeEValue(value, def = 0) {
    var e;
    try {
...
(function() {  //TODO: splash damage (от каждой цели считается ещё доп. цели)
  ...
  _._initMain = function() {
    ...
    //?EVal
    //TODO: Все EVal должны быть NULL по умолчанию, если значение 0
    this.impulse = null;
    this.impulseRandom = 0;
    // * Проверяет что от начальной до конечной точки проходимые тайлы (двигает только по прходимым тайлам)
    this.impulseStrict = 0; //TODO: Implement it
  };
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _.startAASkill = function(aaSkill, subject, targetPoint) {
    ...
    try {
      ...
      if (aaSkill.isRepeatableOnUse()) {
        time = aaSkill.repeatDelay;
        for (i = j = 0, ref = aaSkill.repeatOnUse; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
          setTimeout((function() {
            var e;
            try {
  ...
  _.applySkillAction = function(subject, target, absSkill) {
    ...
    try {
      if (absSkill.isRepeatable()) {
        time = absSkill.repeatDelay;
        for (i = j = 0, ref = absSkill.repeat; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
          setTimeout((function() {
            var e;
            try {
  ...
  _._onActionResult = function(target, action) {
    var battler, d, e, impulseVal, skill;
    try {
      ...
      try {
        if (skill.isHaveImpulsePower()) {
          ...
          if (impulseVal > 0 && target.aaIsCanMoveByImpulse()) {
            ...
            target.aaApplyImpulse(impulseVal, d);
          }
        }
      } catch (error) {
        e = error;
  ...
  _.applyExtraSkillConditions = function(char, targets, absSkill) {
    ...
    try {
      ...
      if (absSkill.isRequireStateOnTarget()) {
        targets = targets.filter(function(t) {
          return (t != null) && t.AABattler().isStateAffected(absSkill.usableIfTargetState);
        });
      }
    } catch (error) {
      e = error;
...
//---------------------------------------------------------------------------
(function() {
  var ALIAS__attackSkillId, ALIAS__changeEquip, ALIAS__forceChangeEquip, ALIAS__forgetSkill, ALIAS__learnSkill, ALIAS__performDamage, ALIAS__setup, _;
  //@[DEFINES]
  ...
  _.meetsABSExtraConditions = function(item) {
    ...
    try {
      ...
      if (aaSkill.isRequireStateOnUser()) {
        return this.isStateAffected(aaSkill.usableIfState);
      }
    } catch (error) {
      e = error;
      AA.w(e);
...
(function() {
  ...
  (function() {    
    // * Методы ABS (Навыки)
    // -----------------------------------------------------------------------
    return _.aaApplyImpulse = function(power, dir = 0) {
      var e;
      try {
        if (power <= 0) {
          return;
        }
        return console.log("APPLY IMPULSE");
      } catch (error) {
        //TODO: random direction, in direction
        //TODO: jump 0, 0 if can't move in direction
        //TODO: move to power cells in direction (if can move)
        e = error;
        return KDCore.warning(e);
      }
    };
...
(function() {
  ...
  (function() {    // -----------------------------------------------------------------------
    ...
    return _.aaApplyImpulse = function(power, dir) {};
  })();
  (function() {    // -----------------------------------------------------------------------
...
(function() {
  ...
  (function() {    // -----------------------------------------------------------------------
    ...
    _.aaIsCanMoveByImpulse = function() {
      return this.isABS() && this.AAModel().heavy !== 1;
    };
  })();
  (function() {    // -----------------------------------------------------------------------
...
(function() {
  ...
  _.aaExecuteEncounter = function(playerRegionId, encTroopId) {
    ...
    try {
      ...
      if ($gameTemp.aaLastSpawnedEvent != null) {
        ...
        return this.aaPlayEncounterAnimation($gameTemp.aaLastSpawnedEvent);
      }
    } catch (error) {
      e = error;
      return KDCore.warning(e);
  ...
  _.aaPlayEncounterAnimation = function(char) {
    var animId, e;
    try {
      animId = this.addGetEncounterAnimation();
      if (animId <= 0) {
        return;
      }
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  (function() {    // * Система АБС (Общее)
    ...
    return _.playExtraAnimation = function(x, y, extraAnimation, seName = "", ox = 0, oy = 0) {
  ...
  (function() {    // * Спавн врагов
    ...
    return _.getSpawnedByEncountersCount = function() {
      var e;
      try {
...
//Plugin Alpha_ABSZ builded by PKD PluginBuilder 2.1 - 12.10.2022