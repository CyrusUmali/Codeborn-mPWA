 ...
 *                 'key' to emulate a keyboard key press
 ...
 *                 'key' action is the key to press like 'a', 'space', etc.
 ...
 * Galv.SBTNS.addButton(5,'map','BtnA',100,300,['key','a']);
 *
 *
...
Galv.SBTNS.getKeyCode = function(key) {
    return this.keyMap[key.toLowerCase()];
};

Galv.SBTNS.keyPress = function(key) {
    var keyCode = Galv.SBTNS.getKeyCode(key);
    if (keyCode) {
        Input._currentState[keyCode] = true;
    }
};

Galv.SBTNS.keyRelease = function(key) {
    var keyCode = Galv.SBTNS.getKeyCode(key);
    if (keyCode) {
        Input._currentState[keyCode] = false;
    }
};
