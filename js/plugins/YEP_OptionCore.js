...
//============================================================================
// Game_Interpreter - Added by Geowil for plugin command to return to last window color
//============================================================================
...
Game_Interpreter.prototype.pluginCommand = function(command, args){
	...
	if (command === 'YEPOpCore'){
		for (var i1 = 0; i1 < args.length; i1++){
			command += " " + args[i1];
		}
	}

	if (command.match(/YEPOpCore SetWindowColor (\d+)/)){
		var matches = (/YEPOpCore SetWindowColor (\d+)/.exec(command) || []);
		updateWindowTone(matches[1]);
	}
}

function updateWindowTone(varID){
    var newTone = $gameVariables._data[varID];
    if (newTone == null) { newTone = [0,0,0,0]; }
	$gameSystem.setWindowTone(newTone);
}
